...
// Setup variables for use
  ...
  Telemetry::GlobalPosition TerminalPosition;
		...
		auto cat  = Popen({"dmesg"}, output{PIPE});
		auto grep = Popen({"egrep", "ttyACM"}, input{cat.output()}, output{PIPE});
		auto cut  = Popen({"cut", "-f3", "-d:"}, input{grep.output()}, output{PIPE});
		auto cut2  = Popen({"tail", "-n1"}, input{cut.output()}, output{PIPE});
		auto res  = cut2.communicate().first;

		std::string re=res.buf.data();
		std::string s ="/dev/";
		s+=re.substr(1,7);
		ArduinoFileIN = open(s.c_str(), O_RDWR | O_NOCTTY);
...
LOOP: do {
  ...
  ofstream myfile;
	...
	{		myfile << "Latitude";
			...
			myfile << "LiDAR";
			myfile << ",\t";
			myfile << "RemainDistance";
			myfile << "\n";
  
   CurrentPosition = vehicle->broadcast->getGlobalPosition();
   auto RemainDistance = CoordinatesToMeters(CurrentPosition.latitude, CurrentPosition.longitude, CurrentPosition.latitude, CurrentPosition.longitude);
	i = 1;	
	while(i<=3){
		distance = readAndSendArduinoDistance(ArduinoFileIN)/100;
		i++;
		}
			myfile << CurrentPosition.latitude;
			myfile << ",\t";
			myfile << CurrentPosition.longitude;
			myfile << ",\t";
			myfile << CurrentPosition.altitude;
			myfile << ",\t";
			myfile << distance;
			myfile << ",\t\t";
			myfile << RemainDistance;
...
//======================================================================
	...
	while(j<=10){
	   ...
	   CurrentPosition = vehicle->broadcast->getGlobalPosition();
	   RemainDistance = CoordinatesToMeters(CurrentPosition.latitude, CurrentPosition.longitude, CurrentPosition.latitude, CurrentPosition.longitude);
		i = 1;	
		while(i<=3){
			distance = readAndSendArduinoDistance(ArduinoFileIN)/100;
			i++;
...
//		auto coordinate = CoordinateToCoordinate(CurrentPosition.latitude, CurrentPosition.longitude, angle, meters);
//		
//		TerminalPosition.latitude = coordinate.first;
//		TerminalPosition.longitude = coordinate.second;
//		
//		auto RemainDistance = CoordinatesToMeters(CurrentPosition.latitude, CurrentPosition.longitude, TerminalPosition.latitude, TerminalPosition.longitude);
...
//		while  (j <= 5)   //( RemainDistance >= 0.2)
//		{
//			CurrentPosition = vehicle->broadcast->getGlobalPosition();
//			std::cout << "Remaining distance from target:"<< RemainDistance << "\n"<< std::endl;			
			
//			if ((CurrentPosition.altitude <= 4) && ( distance <= 1.5) && ( distance >= 0.15)){
//		   	    moveByPositionOffset(vehicle, 50, 0, abs(distance-1.5), 0);
//			}
//			else if ((CurrentPosition.altitude >= 2) && (distance > 1.5)){
//		   	    moveByPositionOffset(vehicle, 50, 0, -abs(distance-1.5), 0);
//			}
//			else 
//			{ 
//		      moveByPositionOffset(vehicle, 50, 0, 0, 0);
//		    }
//		   
//		   CurrentPosition = vehicle->broadcast->getGlobalPosition();
...
//          RemainDistance = CoordinatesToMeters(CurrentPosition.latitude, CurrentPosition.longitude, TerminalPosition.latitude, TerminalPosition.longitude);
...
/*	
  CurrentPosition = vehicle->broadcast->getGlobalPosition();
	...
	if (CurrentPosition.altitude<=0.3 && rc.gear != Up)
	{	
	goto LOOP;	
	}
*/
                     ...
                     float yawThresholdInDeg)
{
  ...
  // mission
  int responseTimeout              = 1;
  int timeoutInMilSec              = 8000;  // initially 10000
